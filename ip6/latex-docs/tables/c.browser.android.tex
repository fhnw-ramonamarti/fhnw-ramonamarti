\begin{table}[ht!] % todo test actions
    \label{table:interactionBrowserAndroid}
    \tablesettings{Vergleich Interaktion Datalist \& Select in Browser (Android, mobile)}
    \footnotesize
    \begin{threeparttable}
        \begin{tabular}{ l || l | l | l }
            ...
            \cline{2-4}    & \tbbr{\textit{Aussen}: Liste schliessen \\ \textit{Innen}: Werte bewegen \\ ohne Änderung} \ccgray & \tbbr{\textit{Aussen}: nichts \\ \textit{Innen}: Werte bewegen \\ ohne Änderung} \ccgray & \tbbr{\textit{Aussen}: Browser-Default\tnote{4} \\ \textit{Innen}: Werte bewegen \\ ohne Änderung} \ccgray \\
            \hline
