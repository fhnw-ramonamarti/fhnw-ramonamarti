...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=100mm]{newComponent/opened.3col.png}
    \caption{\centering Manuelle Test-Komponente}
...
Die genaue Kombination zwischen Browser und Betriebssystemen ist wie folgt: 

\begin{itemize}
    \item \textbf{Mac} bzw. \textbf{OSX}: Chrome (127), Firefox (128), Safari (17.5)
    \item \textbf{Windows}: Chrome (127), Firefox (128), Edge (127)
\end{itemize}

Die SelectComponent aus Abbildung \ref{img:manualTestComponent} zeigt sich in den aufgezählten Kombination grundlegend konsistent. 
...
Auf Windows-PCs mit Nummer-Pad funktionieren beide PageUp-, PageDown-, Home- und End-Keys auf der Tastatur. 
Auf OSX verlangen die oben genannten Tasten und Delete eine Tastenkombination mit der fn-Taste. 

Zusätzlich zu den vereinbarten Systemen und Browser finden noch folgende weitere Prüfungen statt: 
...
Deswegen sind nur Touch-Interaktion möglich. 
...
Diese Interaktion schliesst den Container nicht – anders als der Desktop Safari. 
Ebenfalls bleibt der Container offen, wenn der Klick auf der X einen bereits selektierten Wert löscht. 
Auf dem Pixel Chrome reagiert dafür der Klick auf die Detail-Ansicht abweichend. 
Ist das Popover bereits geöffnet, schliesst die Interaktion dieses und öffnet es gleich wieder. 
Trifft die Touch-Geste das X bei einer bereits getroffenen Selektion, passtiert dieser Fehler nicht.  

Folgende veraltete Browser finden sich in oberflächlichen Tests wieder: 

\begin{itemize}
    \item \textbf{Mac}: Firefox Version 123 => vor Popover API Support
    \item \textbf{iOS}: Safari Version 16.7 => vor Popover API Support
\end{itemize}

% todo old browser

Die breite Abdeckung der manuellen Tests garantiert und beweist bei den abgezielten Browsern eine konsistente Darstellung und einheitliche Interaktionen. 
