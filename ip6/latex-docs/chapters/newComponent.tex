...
Die Benennung des mittleren Rosa mit Accent (\codestyle{--kolibri-color-accent}) bietet einen guten Kontrast zur Selektion. 
...
Die folgenden Screenshots \ref{img:figmaPrototype1} bis \ref{img:figmaPrototype3} zeigen die Prototypen der Dropdown-Komponente: 
...
Nachfolgender Codeausschnitt \ref{code:popoverStyle} zeigt den bei der Gestaltung des Popover- und Option-Styles relevanten Code: 
...
Die \codestyle{@keyframes} regeln die Transformation. 
Das \codestyle{.options-component[popover]} steuert das Layout und das Erscheinungsbild des Popovers. 
...
\codestyle{selectOptionItem}: 
Verschiebt die Klasse \codestyle{.selected} und somit die Hervorhebung auf das neue Element. 

\codestyle{cursorPositionItem}: 
Entfernt bzw. setzt die Klasse \codestyle{.cursor-position} – ähnlich wie beim \codestyle{selectOptionItem}. 

Die entsprechenden CSS-Klassen aus Codeausschnitt \ref{code:eventSelection} steuern die Selektion einer Option.
...
\codestyle{projectSelectedValueOptionView}: 
Erstellt die Ansicht für die ausgewählte Option und verknüpft die Klick-Events mit dem Umschalten des Popovers. 
Diese Kombination von CSS und JavaScript bietet dem Highlighting und der Selektion von Optionen eine visuell ansprechende Umsetzung. 
...
Das \codestyle{.option-item} legt das Erscheinungsbild und die Interaktivität der einzelnen Optionen fest. 
...
Die Nutzung von den CSS-Styles im JavaScript stellt sicher, dass die dynamischen Klassen und Stile ihre Anwendung finden. 
Das Beispiel \ref{code:cssSyleInJs} aus den Projector-Files verwendt die erwänhten folgenden CSS-Klassen: 
...
Das \codestyle{selectProjector.js} erstellt und stylt den Popover-Container und die Detail-Ansicht. 
Das \codestyle{columnOptionsProjector.js} gestaltet die Options-Elemente und die einzelnen Spalten. 
...
Beispiele für die Prototypen und die verschiedenen Zustände sind in der oberen Hälfte des Bildes \ref{img:figmaPrototype1} ersichtlich. 
...
} und das CSS-\codestyle{:has()} nicht unterstützen, zeigen die neue Komponente inkonsistent an. 
...
Im \codestyle{project\-Select\-Views}-Test befindet sich – wie bereits bei der Column – die Prüfungen der Korrektheit der Bindings und View-Elemente. 
Die letzte Datei kümmert sich um die Kontrolle der \codestyle{SelectComponent\-By\-Callbacks} und \codestyle{SelectComponent\-By\-TableValues}. 
