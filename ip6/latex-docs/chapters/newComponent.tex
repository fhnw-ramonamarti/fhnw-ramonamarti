...
\texttt{.cursor-position}: Definiert die Hervorhebung der Option, die gerade durch den Cursor\footnotemark \ fokussiert ist.
\footnotetext{Tastatur-Position-Anker (nicht Maus-Cursor)} % todo check over doc usage cursor
...
Das \texttt{.options-column} aus Code \ref{code:optionStyling} definiert das Layout sowie weitere Listen-abhängige Eigenschaften, 
während das \texttt{.option-item} das Erscheinungsbild und die Interaktivität der einzelnen Optionen festlegt.
...
Das Beispiel \ref{code:cssSyleInJs} aus den Dateien \texttt{selectProjector.js} und \texttt{columnOptionsProjector.js} verwenden die erwänhten CSS-Klassen:
...
% todo update with interaction in table
...
Mit dem Erhalt des Typs \texttt{Option} bietet die Konstante die selbe Funktionalität wie die gewünschten Objekte. 
...
\footnotetext{Offizieller Begriff: Element attached style $\Rightarrow$ Styles direkt im HTML-Element mit Attribut \texttt{style} definiert}
...
\footnotetext{\texttt{!important} besser nicht verwenden}
...
Der Vergleich zwischen Abbildung \ref{img:PerformanceTestBefore} und \ref{img:PerformanceTestAfter} zeigt, dass die Seite um Faktor 13 schneller ist.
