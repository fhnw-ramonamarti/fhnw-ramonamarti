...
Die Komponente nutzt eine mehrstufige Filterung, um die Navigation innerhalb langer Listen zu erleichtern. 
Testpersonen durchlaufen Aufgaben, die auf die Evaluation der Effizienz und Anwenderfreundlichkeit dieser Dropdown-Funktion abzielen. 

 \\
\emph{Aufgabenbeschreibung}: 
\\
\textbf{1. Jahresauswahl}: 
...
Diese Aufgabe dient der Bewertung der Benutzerfreundlichkeit und Effizienz der Filteroptionen. 
\\
\textbf{2. Stadtauswahl}: 
Die Testpersonen wählen eine speziefische Stadt aus einer dreispaltigen Dropdown-Liste. 
...
\begin{itemize}
    \item \textbf{Robert (Junger Alltagsbenutzer, digital native)}: 
          Empfindet die Dropdown-Menüführung als intuitiv und zügig bedienbar. 
    ...
    \item \textbf{Sara (Semi-aufmerksam, nicht so schnell auf der Tastatur)}: 
          Beginnt mit Schwierigkeiten in der Dreispaltenansicht, findet jedoch nach kurzer Einarbeitung die Filteroptionen nützlich. 
    \item \textbf{Nick (Alltagsbenutzer/Student)}: 
          Zeigt sich zufrieden mit der Handhabung und hebt hervor, dass die Filterfunktion das Auffinden des Jahres beschleunigt. 
...
\begin{itemize}
    ...
    \item \textbf{Sara}: 
          Nutzt zuerst nicht alle Filter bei der Stadtauswahl, scrollt somit ein bisschen mehr. 
    \item \textbf{Nick}: 
    ...
    \item \textbf{Reto}: 
          Ist unsicher bei der Nutzung der Filter, erkennt jedoch nach einiger Zeit deren Vorteil. 
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Sara}: 
          Findet die visuelle Gestaltung in Ordnung, vermisst jedoch eine deutlichere Hervorhebung aktiver Filteroptionen. 
    \item \textbf{Nick}: 
          Ist zufrieden mit der Übersichtlichkeit und der klaren Struktur des Drop\-down-Menüs. 
    \item \textbf{Reto}: 
          Findet das Design zunächst verwirrend, besonders in der Dreispaltenansicht, gewöhnt sich jedoch daran. 
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Sara}: 
          Muss sich an die Bedienung gewöhnen, findet diese jedoch nach kurzer Zeit einfach zu handhaben. 
    \item \textbf{Nick}: 
...
\begin{itemize}
    ...
    \item \textbf{Sara}: 
          Findet die dreispaltige zwar nützlich jedoch müssen damit mehrere Spalten bearbeitet werden. 
    \item \textbf{Nick}: 
          Schlägt eine kurze Anleitung oder Hinweistext für Erstnutzer vor. 
    \item \textbf{Reto}: 
          Eventuell eine vereinfachte Variante mit nicht gleich vielen Informationen bei der Länderauswahl. 
\end{itemize}
