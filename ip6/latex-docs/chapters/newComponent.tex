...
% todo
Während das Kolibri-Designsystem eine Vielzahl von Farben bietet, enthält spezifische Anpassungen, um sicherzustellen, dass die Dropdown-Komponente gut lesbar ist.
...
Nachfolgend im Codeausschnitt \ref{code:popoverStyle} ist der relevante CSS-Code, der zur Gestaltung des Popover-Style und Options-Style verwendet wird:

% todo reduzieren auf wesentliche stellen \& beschreiben
% todo code kürzen ev..options-component[popover] inhalt weg kürzen?
% todo caption / label / format
\begin{lstlisting}[style = htmlcssjs, caption = CSS für das Popver-Element, label = code:popoverStyle]
@keyframes open {
    0% {
        transform: scaleY(0);
    }
...
% todo label / format
\begin{lstlisting}[style = htmlcssjs, caption = Aktuelle (cursor-position) und ausgewählte (selected) Option, label = code:styledElement]
.cursor-position {
...
In \texttt{columnOptionsProjector.js} gibt es Event-Handler, die das Setzen und Entfernen der Highlighting-Klassen steuern:
...
// same function for the cursor-position class
\end{lstlisting}

\texttt{cursorPositionItem}: Verändert die Hervorhebung der aktuellen Cursor Position, indem die Klasse \texttt{.cursor-position} auf das neue Element angewendet und vom alten Element entfernt wird.
\texttt{selectOptionItem}: Ähnlich wie \texttt{cursorPositionItem}, aber für ausgewählte Optionen, indem die Klasse \texttt{.selected} gesetzt bzw. entfernt wird.

In \texttt{selectProjector.js} wird das Highlighting hauptsächlich durch die Verknüpfung der Optionen und ihrer Events mit den entsprechenden CSS-Klassen gesteuert.
...
\texttt{projectSelectedValueOptionView}: Diese Funktion erstellt die Ansicht für die ausgewählte Option und verknüpft die Klick-Events mit dem Umschalten des Popovers.
...
\texttt{.options-column} definiert das Layout und die Scroll-Eigenschaften, während \texttt{.option-item} das Erscheinungsbild und die Interaktivität der einzelnen Optionen festlegt.
...
In \texttt{selectProjector.js} wird der Popover-Container erstellt und gestylt, während in \texttt{columnOptionsProjector.js} die einzelnen Options-Elemente gestaltet werden.
