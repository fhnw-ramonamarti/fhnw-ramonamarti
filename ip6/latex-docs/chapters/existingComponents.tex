...
Hier sind die Gegenüberstellungen der bereits vorhandenen Möglichkeiten zur Darstellung einer Optionsauswahl ausgeführt. 
...
Da das Ziel dieses Projekts eine schlanke und einfach verwendbare Kompontente ist, sind die existierenden Frameworks keine Lösung. 
Dieser Bericht legt keinen weiteren Fokus auf extern Bibliotheken. 
...
Safari (OSX und iOS) verwendet als Icon einen Doppelpfeil (nach oben und unten zeigend) wie in Abbildung \ref{img:closedSelectSafari}. 
Firefox zeigt das Feld in grau (Abbildung \ref{img:closedSelectFirefox}), während Chrome und Edge einen weissen Hintergrund wie in Abbildung \ref{img:closedSelectChrome} verwenden. 
...
Die anderen Browser sind einstellungsabhängig und zeigen den Container weiss oder dunkelgrau (Abbildungen \ref{img:openedSelectWin} und \ref{img:openedSelectOsx}) an. 
...
Safari (Mac) und Firefox stellen keinen visuellen Hinweis (Abbildungen \ref{img:closedDatalistSafari} und \ref{img:closedDatalistFirefox}) auf einen Listen-Container dar. 
...
\clearpage
Im Gegensatz zu den oben gezeigten Andriod-Browser zeigen die meisten iOS-Browser die \codestyle{datalist} und die \codestyle{select}s auf der selben Basis an. 
...
Die Gegenüberstellungen der existierenden HTML-Elemente zeigt einige Gemeinsamkeiten, aber auch viele Inkonsistenzen. 
Da die Unterschiede ein Grund für die Entwicklung einer neuen Auswahlkomponente ist, fasst dieser Abschnitt diese nochmals zusammen. 
...
Die Konzentration dieses Abschnitts liegt aber auf der Interaktion, weil das UI bereits im Kapitel \ref{sec:uiDifferences} genauer beschrieben ist. 
...
Während die offene Datalist auf Firefox mit den seitlichen Pfeiltasten zwei verschiedene Reaktionen zeigt, reagiert sie im Chrome-Browser immer auf dieselbe Weise. 
Das Multiselect zeigt in den beiden Browsern bei $\leftarrow$ und $\rightarrow$ ebenfalls inkonsistentes Verhalten. 
Die Enter-Taste löst auf Firefox im geschlossen Single-Select nichst aus, während Chrome die Liste öffnet. 
Dafür reagiert Firefox im Multiselect auf Tab (Input verlassen), wo Chrome kein Verhalten zeigt. 
Die Interaktionen PageUp/-Down zeigen bei Datalist als auch dem Select unterschiedlich. 
...
Im geschlossen Zustand des Single-Select überspringt die Selektion ebenfalls eine unterschiedliche Anzahl Elemente. 
Bei der Interaktion mit der Maus weist das Scrollen auf beiden geöffneten Elementen Inkonsistenzen im Verhalten auf. 

Safari zeigt gegenüber den oben angesprochenen Browsern die grössten Abweichung. 
Einige Aktionen ändern anstelle der Selektion nur das Highlight. 
Am ähnlichsten ist der Apple-Browser zu den Chromium-basierten Browsern wie Chrome. 
...
Die bereits erstellte Länderkomponente löst die Inkonsistenzen und folgt detailliert im nächsten Kapitel. 
