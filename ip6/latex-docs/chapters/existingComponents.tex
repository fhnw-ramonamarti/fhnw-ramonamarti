...
Das Designen von \codestyle{option}s\footnotemark \ beschränkt sich auf die Text- / Hintergrundfarbe, welche jedoch nur in Firefox funktionieren. 
...
Die in Kapitel \ref{sec:implementationsRenderer} definierten Browser unterstützen alle Attribute des Selects - ausser der \codestyle{size}.
...
Der Inhalt\footnotemark \ des Elements können \codestyle{option}s oder \codestyle{optgroup}s sein.
...
Das Stylen der \codestyle{datalist}\footnotemark \ ist sehr begrenzt bzw. nicht möglich. 
...
Bei den textuellen Typen\footnotemark \ existieren \codestyle{text}, \codestyle{search}, \codestyle{number}, \codestyle{email}, \codestyle{url} und \codestyle{tel}.
\footnotetext{\cite{datalistMdn}}
