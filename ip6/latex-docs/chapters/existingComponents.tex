...
Eine Gegenüberstellung die bereits vorhandenen Möglichkeiten zur Darstellung einer Optionsauswahl ist hier aufgeführt.
...
Die typischen Attribute für Eingabefelder wie \texttt{disabled}, \texttt{form}, \texttt{name} und \texttt{required} sind bei einem Select ebenfalls verwendbar.
...
Die in Kapitel \nameref{sec:implementationsRenderer} definierten Browser unterstützen alle Attribute des Selects - ausser der \texttt{size}.
...
Das \texttt{disabled} ermöglicht das Ausgrauen es ganzen Blocks von Auswahloptionen.
...
Allen Browser unterstützen dieses Element.
...
Das Input enthält keinen Inhalt und schreibt sich als selbst-schliessendes Tag.
...
Bei den textuellen Typen\footnotemark \ existieren \texttt{text}, \texttt{search}, \texttt{number}, \texttt{email}, \texttt{url} und \texttt{tel}.
\footnotetext{\cite{datalistMdn}}
...
Es existieren eigene CSS-Pseudo-Klassen, wobei die folgende Aufzählung nur ein Ausschnitt aufzeigt.
