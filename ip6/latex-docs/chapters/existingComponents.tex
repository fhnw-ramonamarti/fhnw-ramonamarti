...
Für eine bessere Accessibility sollte eine $id$ vergeben werden, damit ein Label dem $select$ zugewiesen werden kann.
Standardmässig ist für dieses Feld nur eine einzelne Auswahl möglich.
Durch die Ergänzung des Attribut $multiple$ ist es möglich mehrere Werte zu markieren.
...
Um nach dem Laden der Seite direkt den Fokus auf das Feld zu setzen, kann das Attribut $autofocus$ ergänzt werden.
...
Es gibt jedoch die sehr aufwendige Möglichkeit, den Inhalt zu klonen und durch Wrapper neu zu stylen.
...
Durch die komplexe Struktur des $select$s ist eine eigene Darstellung schwierig zu kontrollieren.
...
In der ARIA-Rolle wird die Komponente als Combobox ohne $size$ und $multiple$ oder Listbox gehandhabt.

Die mehrfache Auswahl rein per Tastatur bietet in der Interaktion geringer Möglichkeiten als mit der Maus.
Mit gedrückter Cmd bzw. Ctrl (Mac bzw. Windows) Taste können mit der Maus weitere Elemente mitausgewählt werden.
...
Zusammengefasst bieten Chromium-basierte Browser die beste Unterstützung für Listen mit den unterschliedlichsten Typen.
...
Die $id$ als globale Eigenschaft dient bei den Eingabe-Felder zusätzlich noch zur Verknüpfung mit einem $label$-Element und somit einer bessern Accessibility.
...
Dieser zeigt in den gängigen Browser nach dem aufklappen ein Palette, welche sich aber in der Darstellung unterscheiden kann. % todo rewrite
...
Grundsätzlich lässt sich das Element in Safari am wenigsten stylen.

Die $datalist$ ist inkonsistenter, da beispielsweise nicht in jedem Browser ein Icon angezeigt wird.
...
Elemente wie Buttons und Listen haben abgerundete Ecken und die Interaktions-Feedbacks können sich leicht von der Windows-Version unterscheiden.
