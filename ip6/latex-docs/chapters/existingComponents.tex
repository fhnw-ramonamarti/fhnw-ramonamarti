...
Alternativ ist das \codestyle{label}-Attribut (Zeile 2 ohne das \codestyle{selected}) zu nutzen. 
...
Diese Variante ist in Zeile 3 (ohne das \codestyle{disabled}) verwendet. 
...
\begin{lstlisting}[style = htmlcssjs, caption = Beispiel – Select, label = code:selectExample]
...
Durch das \codestyle{disabled} (Zeile 1) zeigt sich das Element in einer ausgegrauten Erscheinung. 
...
Der \codestyle{name} (Zeile 1) stellt den Key für das Key-Value-Paar bzw. den Name des Feldes im Formular dar. 
...
Wenn jedoch bei einem Single-Select mehrere Optionen das \codestyle{selected}-Attribut enthalten, gilt das zuletzt Definierte als vorausgewählt.
Das \codestyle{autocomplete} funktioniert wie bei anderen Inputs, indem Vorschläge des User-Agent-Features auftauchen. 
...
Die \codestyle{optgroup} dient als Zusatzelement. 
Das Element gruppiert die Optionen in einem \codestyle{select} und versieht die enthaltenen \codestyle{option}s mit einem Zwischentitel. 
Der Titel lässt sich durch das \codestyle{label}-Attribut – wie in Code \ref{code:optgroupExample} Zeile 2 – setzen, ist aber nicht selektierbar. 
...
Das Element benötigt von den globalen Attributen zumindest eine \codestyle{id} (Zeile 3). 
...
\subsubsection{\color{dblue} Input}
\label{sec:input}
...
Die textlichen Felder\footnote{
    \codestyle{text}, \codestyle{search}, \codestyle{url}, \codestyle{tel}, \codestyle{email}, \codestyle{password}
...
Gewisse Typen – wie z. B. \codestyle{url}, \codestyle{tel} und \codestyle{email} – haben bereits ein Pattern hinterlegt. 
...
Das Select ist in der geschlossenen Form auf der rechten Seite mit mindestens einem Pfeil nach unten ausgestattet. 
...
Je nach Anzahl der enthaltenen Elemente erscheint die Liste darunter (darüber) oder überdeckt die Detail-Ansicht. 
...
Die anderen Browser lassen die Liste bereits beim ersten Klick erscheinen. 
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=80mm]{countryInput-close.png}
...
\begin{figure}[!htb]
    \centering
    \includegraphics[width=80mm]{countryInput-open.png}
...
Ein anderer Anwendungsfall ist die Auswahl eines Jahres oder eines Geburtstags mit drei Selects. 
